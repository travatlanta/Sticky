...
// Shipping type enum for per-product shipping settings
// Allowed values: 'free' (no shipping cost), 'flat' (fixed shipping cost per product),
// and 'calculated' (use automatic/global shipping calculation). When adding new
// products, the default is 'calculated'.
...
export const products = pgTable("products", {
  ...
  templateWidth: integer("template_width").default(1200),
  templateHeight: integer("template_height").default(1200),
  printWidthInches: decimal("print_width_inches", { precision: 6, scale: 3 }).default("4.000"),
  printHeightInches: decimal("print_height_inches", { precision: 6, scale: 3 }).default("4.000"),
  ...
  // Per‑product shipping configuration.  If shippingType is 'flat', a flatShippingPrice
  // must be provided; otherwise it can be null. When shippingType is 'free', shipping
  // is free and flatShippingPrice is ignored. When shippingType is 'calculated', the
  // system will fall back to global or automatically calculated shipping rates.
